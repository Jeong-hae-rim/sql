...
select e.ename 직원이름, d.dname 부서명, to_char(e.sal, '999,999')||'원' 월급
...
select e.empno 직원번호, e.ename 직원이름, e.sal*12 연봉, 
       (sal+nvl(comm, '0'))*12 실급여, s.grade 급여등급
...
select e.ename 이름, nvl(to_char(e.deptno), '0') 부서번호, d.dname 부서이름
from emp e left outer join dept d on (e.deptno = d.deptno);
